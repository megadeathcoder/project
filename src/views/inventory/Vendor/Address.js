...
const handleEditAdd = () => {
  ...
  navigate('/inventory/vendors/address/add',{state: id});
};

const handleEditAddress = (vendor)=>{
  const newVendorId = id;
  console.log('address',vendor);
  vendor.newVendorId = newVendorId ; // Combine vendor object with additional data
...
<table className="table">        
              ...
              <tbody>
              {citiesWithNames.map((vendor) => (
                  <tr key={vendor.id}>
                    <Row>
                      <Col md="6">{vendor.address_line_1}</Col>
                      <Col md="2">{vendor.cityName}</Col>
                      <Col md="2">{vendor.stateName}</Col>
                      <Col md="2"><button type="button" className="btn mybtncustomer btn-secondary btn-sm mr-2" onClick={() => handleEditAddress(vendor)}><i className="bi bi-pencil-fill my-pen-color" /></button><button type="button" className="btn mybtncustomer btn-secondary btn-sm mr-2" onClick={() => handleDeleteClick(vendor.id)}><i className="bi bi-trash-fill my-trash-color" /></button></Col>
