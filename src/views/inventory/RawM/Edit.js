...
const Edit = () => {
  ...
  const {id,name:CityName,unit,category_id:categoryId,sub_category:subCategoryId} = location.state || {}; // Default to an empty object if state is undefined
  ...
  async function apiCall() {
    try {
      console.log('1',formDatas);
        const token = localStorage.getItem('userToken');
        console.log('country_id',formDatas)
        const response = await fetch(`https://factory.teamasia.in/api/public/rawmaterials/${id}`, {
            method: "PUT",
            headers: {
            ...
            body: JSON.stringify({
              name:formDatas.name,
              category_id:formDatas.categoryId,
              sub_category:formDatas.subCategoryId,
              unit:formDatas.unit,
            }),
...
useEffect(() => {
  ...
  const fetchData = async () => {
    ...
    const a1 = result.categories.find(category => category.id === categoryId);
